...
- 端口 `8083`
- 功能：
  - 笔记 CRUD（软删除 + 回收站 + 恢复 + 彻底删除 + 清空回收站）
  - 笔记分类、标签、摘要、富文本内容
  - 笔记内容类型：富文本(HTML) / Markdown 双模式
  - 笔记置顶、收藏
  - 笔记模板：系统预置模板（6个）+ 用户自定义模板，新建笔记时可选择
  - 全局搜索：跨笔记/保险箱/待办的全文检索
   - 密码保险箱 CRUD，后端 AES 加密存储
   - 待办事项 CRUD，支持状态切换、优先级、截止日期
   - 待办邮件提醒：支持提醒时间、提醒邮箱
   - 用户操作活动日志：记录笔记/保险箱/待办的所有操作，支持统计和按天查询
   - 日历日程管理：事件 CRUD、月视图/周视图查询、日期范围查询、全天事件、颜色标签、地点
   - 知识库 Wiki：多知识库空间管理、树形页面结构、无限层级子页面、页面内容编辑
   - 文件库：文件上传至服务器本地目录（`/data/note-files/{userId}/`）、下载、预览、虚拟文件夹分类、搜索、存储统计
   - 工作台 Dashboard：全模块数据总览、本周效率报告、知识增长曲线、活动热力图、优先待办列表
...
- `sql/note_migration_20260412.sql`
- `sql/todo-reminder-upgrade.sql`
- `sql/note_upgrade_20260501.sql`（笔记软删除 + 活动日志表）
